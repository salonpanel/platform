...
interface AgendaQuickActionsProps {
  ...
  className?: string;
  /**
   * `notifications`: solo campana (cabecera móvil izquierda).
   * `searchAndFilters`: búsqueda + filtros (cabecera móvil derecha).
   * Sin definir: fila completa (desktop).
   */
  only?: "notifications" | "searchAndFilters";
}
...
export function AgendaQuickActions({
  ...
  if (only === "notifications") {
    return <div className={cn("flex items-center flex-shrink-0", className)}>{notificationButton}</div>;
  }

  if (only === "searchAndFilters") {
    return (
