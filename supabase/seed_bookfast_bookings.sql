...
BEGIN
  ...
  RAISE NOTICE '🚀 Generando reservas BookFast desde % hasta %', v_start_date, (v_start_date + INTERVAL '2 years');
  ...
  WHILE v_current_date <= v_start_date + INTERVAL '2 years' LOOP
    ...
    v_base_probability := 0.55;
    IF v_day_of_week IN (4,5) THEN
    ...
    FOR v_staff_id IN 
      SELECT DISTINCT staff_id FROM public.staff_schedules ss
      WHERE ss.tenant_id = v_tenant_id AND ss.day_of_week = v_day_of_week AND ss.is_active = true
    LOOP
      FOR v_booking_time IN
        SELECT (ss.start_time + (interval '30 minutes' * gs))::time
      ...
      LOOP
        IF random() < v_base_probability THEN
          ...
          IF v_current_date < CURRENT_DATE - INTERVAL '1 day' THEN
            v_status := CASE WHEN random() < 0.85 THEN 'completed' WHEN random() < 0.93 THEN 'confirmed' WHEN random() < 0.97 THEN 'cancelled' ELSE 'no_show' END;
          ELSIF v_current_date < CURRENT_DATE THEN
            v_status := CASE WHEN random() < 0.9 THEN 'confirmed' ELSE 'completed' END;
          ELSE
            v_status := 'confirmed';
          END IF;

          IF NOT EXISTS (
            SELECT 1 FROM public.staff_blockings sb
            WHERE sb.tenant_id = v_tenant_id
              AND sb.staff_id = v_staff_id
              AND tstzrange(sb.start_at, sb.end_at, '[)') && tstzrange(v_starts_at, v_ends_at, '[)')
          ) THEN
            BEGIN
              INSERT INTO public.bookings (tenant_id, customer_id, staff_id, service_id, starts_at, ends_at, status, duration_min, is_highlighted, internal_notes, created_at)
              VALUES (v_tenant_id, v_customer_id, v_staff_id, v_service_id, v_starts_at, v_ends_at, v_status, v_duration, random() < 0.05, CASE WHEN random() < 0.2 THEN 'Cliente recurrente' ELSE NULL END, v_starts_at - INTERVAL '2 days');
              v_count := v_count + 1;
            EXCEPTION WHEN exclusion_violation THEN NULL; WHEN OTHERS THEN RAISE NOTICE 'Error insertando booking: %', SQLERRM; END;
          END IF;
        END IF;
      END LOOP;
    END LOOP;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Reserva demo error: %', SQLERRM;
END;
$$;
