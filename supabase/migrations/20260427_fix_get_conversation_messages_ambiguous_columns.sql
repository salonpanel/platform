-- RETURNS TABLE(...) crea variables PL/pgSQL (id, created_at, …) que colisionan con
-- columnas en subconsultas → "column reference created_at is ambiguous".
...
BEGIN
  ...
  IF p_before_timestamp IS NOT NULL THEN
    SELECT MIN(w.created_at), MAX(w.created_at)
  ...
  ELSIF p_after_timestamp IS NOT NULL THEN
    ...
    FROM (
      ...
      FROM public.team_messages msg
      WHERE msg.conversation_id = p_conversation_id
        AND msg.created_at > p_after_timestamp
        AND msg.deleted_at IS NULL
  ...
  SELECT EXISTS(
    SELECT 1 FROM public.team_messages msg
    WHERE msg.conversation_id = p_conversation_id
      AND msg.created_at < COALESCE(v_oldest_timestamp, NOW())
      AND msg.deleted_at IS NULL
  ...
  SELECT EXISTS(
    SELECT 1 FROM public.team_messages msg
    WHERE msg.conversation_id = p_conversation_id
      AND msg.created_at > COALESCE(v_newest_timestamp, '1970-01-01'::TIMESTAMPTZ)
      AND msg.deleted_at IS NULL
