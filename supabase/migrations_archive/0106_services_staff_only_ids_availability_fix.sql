...
-- Corrige la función get_available_slots para incluir la lógica completa de staff_only_ids

begin;

-- Recrear la función completa con el filtro de staff_only_ids
...
begin
  ...
  staff_schedules as (
    ...
    -- FILTRAR por staff_only_ids si existe restricción, o por provides_services si no
    select
      s.staff_id,
      s.day_of_week as weekday,
      s.start_time,
      s.end_time,
    ...
    where s.tenant_id = p_tenant_id
      ...
      -- Lógica de asignación: staff_only_ids tiene prioridad, si no existe usa provides_services
      and (
        (v_staff_only_ids is not null and array_length(v_staff_only_ids, 1) > 0 and s.staff_id = any(v_staff_only_ids))
        or
        (v_staff_only_ids is null or array_length(v_staff_only_ids, 1) is null) and st.provides_services = true
      )
  ),
