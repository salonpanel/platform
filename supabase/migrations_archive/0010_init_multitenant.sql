...
BEGIN
  IF to_regclass('public.schedules') IS NOT NULL THEN
    ...
    -- Eliminar policies si existen (para evitar conflictos)
...
CREATE INDEX IF NOT EXISTS idx_bookings_tenant_starts ON public.bookings(tenant_id, starts_at, status);
CREATE INDEX IF NOT EXISTS idx_bookings_tenant_customer ON public.bookings(tenant_id, customer_id);
CREATE INDEX IF NOT EXISTS idx_bookings_tenant_staff_starts ON public.bookings(tenant_id, staff_id, starts_at);
...
-- Users
DROP POLICY IF EXISTS "tenant_read_users" ON public.users;
CREATE POLICY "tenant_read_users" ON public.users
  FOR SELECT 
  USING (tenant_id = app.current_tenant_id());

DROP POLICY IF EXISTS "tenant_modify_users" ON public.users;
